...
def normalize(pc, margin=0.01):
    ...
    center = torch.Tensor([(x.max()+x.min())/2, (y.max()+y.min())/2, (z.max()+z.min())/2])
    longest = torch.max(torch.Tensor([x.max() - x.min(), y.max() - y.min(), z.max() - z.min()]))
...
class PositionalEncoding3d(nn.Module):
    ...
    def forward(self, pos):
        if self.pe_embed == 'none':
            ...
            pe_list = []
            for i in range(self.levels):
                temp_value = pos[0] * self.lbase **(i) * math.pi+pos[1] * self.lbase **(i) * math.pi+pos[2] * self.lbase **(i) * math.pi
                pe_list += [torch.tensor([torch.sin(temp_value)],dtype=torch.float32), torch.tensor([torch.cos(temp_value)],dtype=torch.float32)]
