...
def quant_weights(w, args):
    ...
    if args.wei_quant_scheme.startswith('uni'):
        ...
        w_int,qw = uniform_symmetric_quantizer(w, bits=args.wei_bits)
        w_int = (w_int)
    else: 
        try:
        ...
        w_int,qw, bkp_ratio, _ = piecewise_linear_quant(w, 
    ...
    tail_num = np.sum(torch.abs(w).cpu().detach().numpy() > float(break_point))

    return w_int,qw, err, tail_num