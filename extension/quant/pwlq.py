...
def piecewise_linear_quant(w, bits=4.0, scale_bits=0.0, 
    ...
    if break_point_approach == 'search': 
        ...
        w_int,qw, bkp_ratio, err = pwlq_coarse2fine_grid_search(w, 
        ...
        w_int,err, qw = pwlq_quant_error(w, bits, scale_bits, abs_max, break_point, pw_opt)
    return w_int,qw, bkp_ratio, err
...
def pwlq_quant_error(w, bits, scale_bits, abs_max, break_point, pw_opt):
    ...
    ## option 1: overlapping
    if pw_opt == 1:
        w_int_tail,qw_tail = uniform_symmetric_quantizer(w, 
            bits=bits, scale_bits=scale_bits, minv=-abs_max, maxv=abs_max)
        w_int_middle,qw_middle = uniform_symmetric_quantizer(w, 
        ...
        w_int = (w_int_tail,w_int_middle)
        
    
    ## option 2: non-overlapping
    if pw_opt == 2:
        w_int_tail_neg,qw_tail_neg = uniform_affine_quantizer(w, 
            bits=bits-1, scale_bits=scale_bits, minv=-abs_max, maxv=-break_point)
        w_int_tail_pos,qw_tail_pos = uniform_affine_quantizer(w, 
        ...
        w_int = (w_int_tail_neg,w_int_middle,w_int_tail_pos)
...
def pwlq_coarse2fine_grid_search(w, bits=4.0, scale_bits=0.0, search_range=10, pw_opt=2):
    ...
    return w_int,best_qw, best_ratio, min_err
